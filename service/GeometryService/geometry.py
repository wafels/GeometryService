...
class GeometrySpiceError(Fault):
    ...
    def __init__(self, message):
        Fault.__init__(
            self, faultcode="Client.SpiceError", faultstring="SPICE Error: %r" % message
        )
...
def distribute_work(func_spice, xfunc, tais, **kwargs):
    ...
    results = [
        apply_async(
            pool,
            func_spice,
            args=(xfunc, tais[chunk * i : chunk * (i + 1)]),
            kwds=kwargs,
        )
...
def state_internal(
    func_spice, utc, utc_end, deltat, kind, observer, target, ref, abcorr
):
    try:
    ...
    kwargs = {"observer": obs, "target": tgt, "ref": ref, "abcorr": abcorr}
...
def position(utc, utc_end, deltat, kind, observer, target, ref, abcorr):
    return state_internal(
        position_spice, utc, utc_end, deltat, kind, observer, target, ref, abcorr
    )


def state(utc, utc_end, deltat, kind, observer, target, ref, abcorr):
    return state_internal(
        state_spice, utc, utc_end, deltat, kind, observer, target, ref, abcorr
    )
